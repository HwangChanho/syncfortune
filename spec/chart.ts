...
export interface Interaction {        // 합·충·형·해·파 — *검출은 결정론(RULE)*
  ...
  /**
   * ★참여한 글자의 **출처**(R46 개정 스펙 4번 '파싱 무결성', 2026-08-15).
   *
   * 왜 필요한가: 궁합은 두 사람의 글자가 섞이므로 `members`(자리 이름)만으로는
   * **누구의 어느 기둥인지** 되짚을 수 없다. 실제로 수기 판단 중 상대의 卯 개수를
   * 2개로 오독해 결론이 뒤집힌 일이 있었다 — 그런 오독을 코드가 잡아내려면 출처가 붙어야 한다.
   *
   * 규약: 추론·요약본이 아니라 **파싱된 차트 객체에서 그대로** 채운다.
   * 원국 단독 검출에서는 비어 있어도 된다(`members` 로 충분하다).
   */
  refs?: {
    /** 어느 쪽 명식인가 — 궁합에서만 의미가 있다 */
    chart: 'self' | 'other';
    /** 그 명식의 몇 번째 기둥인가 */
    pillar: '년' | '월' | '일' | '시';
    /** 실제 글자(역추적 시 대조용 — 자리만 믿지 않는다) */
    char: string;
  }[];
}
