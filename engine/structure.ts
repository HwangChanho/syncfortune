...
export function scoreStrength(saju: SajuChart): { score: number; verdict: '신강' | '중화' | '신약'; breakdown: string[] } {
  ...
  //   삼합·방합국(3자 완성): 화기 오행이 우호(비겁·인성)면 세력 결집(+), 비우호면 일간 약화(-).
  ...
  for (const it of (saju.interactions ?? [])) {
    ...
    if (it.type === '합' && it.members.length >= 3 && it.transformsTo) {     // 삼합·방합국
      const adj = favor.has(it.transformsTo) ? GUK_BONUS : -GUK_BONUS;
