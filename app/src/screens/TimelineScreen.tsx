    ...
    if (!r) {
      ...
      return (
        <View style={[styles.card, styles.lockCard]}>
          ...
          <Text style={[styles.lockSub, { fontSize: fs(12), lineHeight: 19 }]}>{t('timeline.lockedSub')}</Text>
    ...
    return (
      <View style={styles.card}>
        ...
        {typeof r.headline === 'string' && r.headline.trim() ? (
          <Text style={{ fontSize: fs(19), fontWeight: '800', color: colors.ju, marginBottom: space(3), lineHeight: 26 }}>{r.headline}</Text>
        ) : null}
        {/* 카테고리 칩 — 대운 5 / 연운 8 */}
...
const styles = StyleSheet.create({
  ...
  row: { minHeight: 48, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: space(2), borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line },
