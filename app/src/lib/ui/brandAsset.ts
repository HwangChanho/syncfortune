...
/**
 * 브랜드 로고(심볼만) — **누끼**(투명 배경).
 * ⚠️바탕 위에 그대로 올리면 얇은 선이 배경에 묻힌다. 밝은 라벤더 배경 위에서는 특히 그렇다.
 *   ⇒ 로고를 **하나만** 크게 보여 주는 자리(로그인·온보딩)에는 `brandMarkSolid()` 를 쓴다.
 */
export const brandMark = () => A(`${BRAND_DIR}/mark.png`);
/**
 * 브랜드 로고 — **배경이 있는 판**(앱 아이콘과 같은 그림).
 *
 * Boss 2026-08-24 *"웹에서 뜨는 로고는 누끼 없는걸로 해야지"*.
 * 로고가 화면의 주인공인 자리(로그인·온보딩)에서 쓴다. 앱 아이콘과 **같은 파일**을 보므로
 * 아이콘을 바꾸면 여기도 같이 바뀐다(따로 관리하지 않는다).
 * ★둥근 모서리는 쓰는 쪽 스타일에서 준다 — 자산에 굽지 않는다(자리마다 반경이 다르다).
 */
export const brandMarkSolid = () => require('../../../assets/icon.png');
