...
/**
 * ★동음이의 오탐 차단(daniel 2026-07-30 IMG_8305 "저 세운이 왜 볼드처리 됐는지 모르겠고").
 *   "나 자신을 단단히 **세운** 상태에서" 의 '세운'은 동사 '세우다'의 활용형인데
 *   명리 용어 '세운(歲運)' 으로 잡혀 볼드가 됐다.
 *
 *   구분 규칙: 명리 용어는 **조사가 붙어** 끝난다(세운은/세운이/세운의…).
 *   동사 활용형은 뒤에 **명사**가 온다(세운 상태 / 세운 계획) — 즉 공백 뒤 다른 말이 이어진다.
 *   ⚠️그래서 위험 단어는 **공백을 허용하지 않는다**. "올해 세운 丙午" 같은 표기는 강조를 놓치지만,
 *     엉뚱한 볼드보다 낫다(오탐이 신뢰를 더 깎는다).
 *   ⚠️동음이의가 없는 단어(대운·원국·용신…)까지 조이면 "대운 흐름" 같은 정상 표기를 놓친다
 *     → **위험 단어에만** 엄격 규칙을 적용한다.
 */
const AMBIGUOUS = ['세운', '기신', '식상', '한신'];   // 세우다 / 기신거리다 / 식상하다 / (지명)
const JOSA = String.raw`(?=은|는|이|가|을|를|에|의|으로|로|과|와|도|만|부터|까지|처럼|보다|라도|이나|[,.!?)\]"'·…]|$)`;

const TERM_PATTERN = new RegExp(
  [...TERM_WORDS].sort((a, b) => b.length - a.length)
    .map((w) => (AMBIGUOUS.includes(w) ? `${w}${JOSA}` : w))
    .join('|'),
  'g',
);
