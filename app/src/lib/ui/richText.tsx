...
//
// ★★2026-08-26 — **겹별표 `**…**` 를 더했다** (Boss *"대화에서 강조를 ** 이걸로하는데
//   이걸로 그대로나오는데 이게 Bold 처리가 돼야지"*).
//   LLM 은 마크다운을 배워서 **겹별표**를 쓴다. 그리고 한글로 시작하지 않는 것도 강조한다
//   (실물: `**경오일주(庚午日)**`). 종전 규칙(홑별표 + 한글 시작)으로는 **하나도 안 잡힌다.**
//   ⇒ 겹별표를 **먼저** 시도한다 — 순서를 바꾸면 홑별표 규칙이 `**abc**` 의 앞 별표 하나를
//     먹어 «*abc*» 가 남는 이상한 결과가 된다.
//   ⚠️기존 홑별표 규칙은 **그대로 둔다.** 그건 온디바이스 콘텐츠(daniel 이 직접 쓴 문자열)용이고,
//     «한글로 시작할 때만» 이라는 제약이 각주·불릿·곱셈 오변환을 막고 있다. 건드리면 그게 깨진다.
//   ★겹별표는 그 제약을 두지 않는다 — 한자·괄호·숫자도 강조 대상이고, 짝이 분명해서 오변환 위험이 없다.
const EMPH_RE = /\*\*([^*\n]+?)\*\*|\*([가-힣][^*]*)\*/g;
...
function splitEmphasis(input: string): Seg[] {
  ...
  while ((m = EMPH_RE.exec(input)) !== null) {
    ...
    segs.push({ bold: true, text: m[1] ?? m[2] ?? '' }); // 강조부 — m[1]=겹별표 · m[2]=홑별표(한글 시작)
