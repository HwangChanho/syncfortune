...
// ★2026-08-25 판정 요청함 → `verify-000t-taegil`(9문항). 답이 오면 `check:stance` 가 먼저 운다.
// ─────────────────────────────────────────────────────────────────────────
...
// 묘고지(墓庫支) 辰戌丑未 — 평소 지장간이 잠복하다가 *충을 맞아 열릴 때(개고)* 튀어나온다(daniel stance).
...
function scoreDay(saju: SajuChart, dayStem: string, dayBranch: string, purpose: Purpose): { score: number; reasons: string[] } {
  ...
  // ③ 목적별 선호 십신 — 그날 천간(드러난 기운)만. ★지장간은 *평소 잠복*이라 여기서 보지 않는다(daniel) — 아래 개고(③-2)로만.
  let pref = 0;
  const surf = GROUP[tenGod(me as any, dayStem as any)];
  if (surf) pref += PURPOSE_PREF[purpose][surf] ?? 0;
  // ③-2 개고(開庫) — 묘고지(辰戌丑未)는 *충을 맞아 열릴 때* 지장간이 튀어나온다(daniel). 그날 지지가
  //   내 일지/운지의 묘고지를 충하면, 열린 지장간 십신을 목적 선호에 *온전히* 반영(평소 잠복 → 운에서 발현).
  const openable = [myBranch, ...luckBranches].filter((b) => STORAGE.has(b) && CHUNG[b] === dayBranch);
  for (const tb of openable) {
    for (const h of (HIDDEN[tb as keyof typeof HIDDEN] ?? [])) {
      const g = GROUP[tenGod(me as any, h.stem as any)];
      if (g) pref += PURPOSE_PREF[purpose][g] ?? 0;
    }
  }
  if (pref > 0) { score += pref; reasons.push(R.fit(purposeLabel(purpose))); }

  // ④ 역마(이동)·도화(이성 끌림) — 내 일지 삼합국 기준(일지가 그날 운과 만날 때 발현, daniel). 목적에 해당할 때만 가점.
