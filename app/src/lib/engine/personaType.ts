...
const GYEOK: Record<string, { label: string; axis: string; kw: string }> = {
  ...
  // ★비겁 월지 특칙(daniel 2026-07-28 stance) — 격 이름이 '건록격/월겁격'으로 바뀌었다.
  //   이 키가 없으면 `GYEOK[gyeok]` 이 undefined 가 되어 **성격유형 카드에서 격 라벨이 통째로 사라진다**
  //   (표본 216 기준 약 16%). 에러도 안 나고 문장만 짧아져서 알아채기 어려운 회귀다.
