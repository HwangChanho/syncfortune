...
export function ReunionRich({ saju }: { saju: SajuChart }) {
  ...
  // ⚠️★`t` 를 의존성에 넣는다 — 언어가 바뀌면 **다시 계산해야** 문구가 따라온다.
  const d = useMemo(() => {
    ...
    const gaugeCaption = tone === 'open'
      ? t('rr.capOpen', '옛 인연과 다시 이어질 문이 열려 있는 흐름이에요. 마음이 있다면 지금 진심을 전해 보세요.')
      : tone === 'warming'
        ? t('rr.capWarming', '문이 조금씩 열리는 중이에요. 서두르기보다 결이 무르익는 시기를 노려 보세요.')
        : t('rr.capQuiet', '지금은 문이 잠잠한 편이에요. 무리한 연락보다 나를 가꾸며 때를 기다리는 게 좋아요.');

    // ② 배우자궁 개폐 '결' 문구(일상어)
    const gungLabel = gungOpen ? t('rr.gungOpen', '지금 열리는 결이에요')
      : gungBond ? t('rr.gungBond', '지금 맺히는 결이에요')
        : gungFriction ? t('rr.gungFriction', '지금 살짝 부딪히며 움직이는 결이에요')
          : t('rr.gungCalm', '지금은 잠잠한 결이에요');
    const gungSub = gungOpen ? t('rr.gungOpenSub', '인연의 자리가 열리는 시기예요. 다시 마주할 계기가 생기기 쉬워요.')
      : gungBond ? t('rr.gungBondSub', '인연의 자리가 이어지려는 결이에요. 관계를 다시 매듭짓기 좋은 때예요.')
        : gungFriction ? t('rr.gungFrictionSub', '자리가 흔들리며 움직여요. 감정보다 차분함으로 다가가면 결이 부드러워져요.')
          : t('rr.gungCalmSub', '자리가 아직 고요해요. 억지로 열기보다 나를 채우며 다음 흐름을 기다려요.');
    ...
    const dirSeason = natalDohwa.length ? (() => {
      ...
      const seasons = [...new Set(natalDohwa.map((x) => t(DOHWA_DIR[x as '子' | '午' | '卯' | '酉'].season)))];
      const dir = t(DOHWA_DIR[(primary ?? natalDohwa[0]) as '子' | '午' | '卯' | '酉'].dir);
      return { seasons, dir };
    })() : null;
  ...
  return (
    <>
      ...
      <PossibilityGauge score={d.score} label={d.tendency} tone={d.tone} title={t('rr.gaugeTitle', '재회의 문이 열린 정도')} caption={d.gaugeCaption} />

      {/* ② 배우자궁 개폐 상태 */}
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t('rr.seatTitle', '인연의 자리')}</Text>
      ...
      {d.dirSeason && (
        <View style={styles.card}>
          <Text style={styles.cardTitle}>{t('rr.dirTitle', '인연 기운의 방향·계절')}</Text>
          <Text style={styles.cardBody}>
            <Text>{t('rr.dirBodyA', '당신의 인연 기운은 ')}</Text><Text style={styles.accent}>{d.dirSeason.seasons.join('·')}</Text><Text>{t('rr.dirBodyB', '에 무르익고, ')}</Text><Text style={styles.accent}>{d.dirSeason.dir}</Text><Text>{t('rr.dirBodyC', ' 방향의 기운과 잘 통해요.')}</Text>
          </Text>
        </View>
      )}
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t('rr.gaeunTitle', '연락 개운 미리보기')}</Text>
        <View style={styles.teaserRow}>
          <Text style={styles.teaserLabel}>{d.teaser.label}</Text>
          <Text style={styles.teaserValue}>{d.teaser.value}</Text>
        </View>
        <Text style={styles.lockNote}>🔒 {t('rr.locked', '좋은 방위 · 나머지 색·요일 · 구체 실천법은 깊은 풀이에서')}</Text>
        <View style={styles.divider} />
        {/* 무료 vs 유료 잠긴 가치 명시(퍼널 훅 — 골드 CTA 바로 위) */}
        <Text style={styles.funnelLine}><Text>{t('rr.freeA', '무료로는 ')}</Text><Text style={styles.accent}>{t('rr.freeB', '가능성·시기·방향')}</Text><Text>{t('rr.freeC', '까지 볼 수 있어요.')}</Text></Text>
        <Text style={styles.funnelLine}><Text>{t('rr.paidA', '깊은 풀이에선 ')}</Text><Text style={styles.accent}>{t('rr.paidB', '상대의 마음·구체 개운법·다시 이어질 이유')}</Text><Text>{t('rr.paidC', '까지 짚어 드려요.')}</Text></Text>
      </View>
    </>
  );
}
