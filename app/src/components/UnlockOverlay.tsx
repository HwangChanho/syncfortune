...
// (로딩 영상 전면 제거 2026-08-05 — mp4 require 를 남기면 번들에 계속 실린다. 5종 ~수 MB 절감)
...
export function UnlockOverlay({ visible, message, allowBackground = true, videoKey: videoKeyProp, minMs = 3200, done, total }: { visible: boolean; message?: string; allowBackground?: boolean; videoKey?: VideoKey; minMs?: number;
  ...
  // ★로딩 영상 전면 제거(daniel 2026-08-05 "풀이 로딩영상이랑 로딩화면 영상 다 없애버려").
  //   prop·호출부(15곳)는 그대로 두고 여기서 무조건 undefined — 영상 경로 전체가 죽고 링+자물쇠만 남는다.
  //   (설정의 풀이영상 토글·videoKeyProp 은 무시된다. 자산 정리는 별도 커밋에서.)
  void videoKeyProp; // prop 은 호환용으로만 받는다(영상 전면 제거)
  ...
  return (
    ...
    <View style={[StyleSheet.absoluteFill, styles.overlay, styles.rootZ]}>
        {/* 회전 골드 링 + 자물쇠(🔒→🔓) 펄스 — 영상 모드는 2026-08-05 전면 제거 */}
