...
//   ⚠️더 줄이면 「함께하면 좋은 친구들」 같은 긴 라벨이 아니라 **워드마크**가 먼저 걸린다(21px × 4자).
const SIDEBAR = 208;
...
const WIDE_ROUTES = ['/', '/chats', '/talk', '/contents', '/category', '/relationmap', '/charts', '/market', '/community'];
/**
  ...
  return (
    <View style={styles.side}>
      ...
      <View style={styles.brand}>
        {/* ★★2026-08-26 Boss *"웹 제일 왼쪽에 니운내운 왼쪽에 로고가 들어가면 좋겠어"*
            — **여기가 «제일 왼쪽»** 이다(대화 목록 헤더가 아니라 이 사이드바).
            심볼은 파비콘과 **같은 원본**에서 뽑은 투명 「운」 이다(배경 네모가 없다). */}
        <BrandWordmark symbol />
      </View>
      {tabs.map((tb) => {
