...
import { computeInyeonSignals } from '../lib/love/inyeonGauge';
import { GAEUN } from '../lib/love/gaeunTable';   // ★개운 표 단일 출처(재회와 공유 — 종전엔 여기 **사본**이 있었다)
import { useTranslation } from 'react-i18next'; // 인연 게이지 결정론 신호(재회·애정·짝사랑 공통 엔진)
...
export function CrushRich({ saju }: { saju: SajuChart }) {
  ...
  const d = useMemo(() => {
    ...
    const gaugeCaption = tone === 'open'
      ? t('cr.capOpen', '지금 그 사람과 이어질 흐름이 활짝 열려 있어요. 마음이 있다면 자연스럽게 다가가 보세요.')
      : tone === 'warming'
        ? t('cr.capWarming', '이어질 흐름이 서서히 데워지고 있어요. 서두르기보다 결이 무르익는 달을 노려 마음을 살짝 표현해 보세요.')
        : t('cr.capQuiet', '지금은 흐름이 차분한 편이에요. 조급해하기보다 나를 가꾸며 끌림이 도는 때를 준비하면 좋아요.');

    // ② 끌림 신호 (가) 도화 발동(합/충) = 일지(인연 자리) vs 지금 운의 충(열림)/합(맺힘)/형(엇갈림) '결'(일상어)
    const dohwaLabel = gungOpen ? t('cr.dohwaOpen', '지금 끌림이 강하게 도는 결이에요')
      : gungBond ? t('cr.dohwaBond', '지금 인연이 맺히려는 결이에요')
        : gungFriction ? t('cr.dohwaFriction', '지금 살짝 엇갈리며 움직이는 결이에요')
          : t('cr.dohwaCalm', '지금은 조용한 결이에요');
    const dohwaSub = gungOpen ? t('cr.dohwaOpenSub', '설렘과 계기가 강하게 이는 시기예요. 마음을 표현하면 통하기 쉬워요.')
      : gungBond ? t('cr.dohwaBondSub', '서로 가까워지려는 결이에요. 가볍게 다가가면 자연스럽게 이어지기 좋아요.')
        : gungFriction ? t('cr.dohwaFrictionSub', '밀고 당기는 결이에요. 조급함보다 여유로 다가가면 흐름이 부드러워져요.')
          : t('cr.dohwaCalmSub', '끌림이 아직 은근해요. 무리하기보다 나를 가꾸며 다음 흐름을 기다려요.');
    ...
    const charmLabel = hasHongyeom ? t('cr.charmBorn', '타고난 끌림(매력)이 있어요') : t('cr.charmSubtle', '은근하게 스며드는 매력이에요');
    const charmSub = hasHongyeom
      ? t('cr.charmBornSub', '은은한 분위기로 사람을 끄는 매력이 있어, 마음을 표현하면 상대가 끌리기 쉬워요.')
      : t('cr.charmSubtleSub', '화려하기보다 서서히 스며드는 매력이라, 자주 마주치며 편안함을 쌓으면 마음이 열려요.');

    // ④ 다가가기 개운 티저 — 색/요일 중 딱 하나만 공개(방위는 잠금 → 유료 가치 보존). 일간으로 결정론 선택.
    const gaeun = GAEUN[inyeonEl];
    const teaser = ALL_STEMS.indexOf(dm) % 2 === 1
      ? { label: t('cr.teaserDow', '다가가기 좋은 요일'), value: t(gaeun.day) }
      : { label: t('cr.teaserColor', '다가가기 좋은 색'), value: t(gaeun.color) };
  ...
  return (
    <>
      ...
      <PossibilityGauge score={d.score} label={d.tendency} tone={d.tone} title={t('cr.gaugeTitle', '그 사람과 이어질 가능성')} caption={d.gaugeCaption} />

      {/* ② 끌림 신호 — 도화 발동(합/충)[일지 개폐] + 홍염(타고난 매력)[일간 파생] */}
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t('cr.signalTitle', '끌림 신호')}</Text>
        {/* (가) 지금 도화(끌림)가 도는 결 */}
      ...
      <View style={styles.card}>
        <Text style={styles.cardTitle}>{t('cr.gaeunTitle', '다가가기 개운 미리보기')}</Text>
        <View style={styles.teaserRow}>
          <Text style={styles.teaserLabel}>{d.teaser.label}</Text>
          <Text style={styles.teaserValue}>{d.teaser.value}</Text>
        </View>
        <Text style={styles.lockNote}>🔒 {t('cr.locked', '좋은 방위 · 나머지 색·요일 · 다가가는 구체 실천법은 깊은 풀이에서')}</Text>
        <View style={styles.divider} />
        {/* 무료 vs 유료 잠긴 가치 명시(퍼널 훅 — 골드 CTA 바로 위) */}
        <Text style={styles.funnelLine}><Text>{t('rr.freeA', '무료로는 ')}</Text><Text style={styles.accent}>{t('cr.freeB', '가능성·시기·다가갈 타이밍')}</Text><Text>{t('rr.freeC', '까지 볼 수 있어요.')}</Text></Text>
        <Text style={styles.funnelLine}><Text>{t('rr.paidA', '깊은 풀이에선 ')}</Text><Text style={styles.accent}>{t('cr.paidB', '그 사람 마음·다가가는 법·이어질 이유')}</Text><Text>{t('rr.paidC', '까지 짚어 드려요.')}</Text></Text>
      </View>
    </>
  );
}
