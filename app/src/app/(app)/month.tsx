...
export default function MonthScreen() {
  ...
  async function generate(id: string) {
    ...
    try {
      const __inv = await withTimeout(supabase.functions.invoke('interpret', {
        body: { chartId: id, category, kind: 'monthly', gz: f.monthGanZhi, tier: 'paid', lang: readingLang(), ...(saved?.context ? { context: saved.context } : {}) },
      }), GEN_TIMEOUT_MS);
  ...
  async function onStart() {
    ...
    try { earned = (await withTimeout(showRewardedAd(), 15_000)) ?? false; } catch { /* 미시청/닫기 */ }
    setBusy(false);
