...
export default function ContentsScreen() {
  ...
  //   ★상태를 **화면이** 갖는다 — 검색창을 스크롤 밖 최상단에 고정해야 하기 때문(아래 주석 참조).
  const [q, setQ] = useState('');
  const searching = q.trim().length > 0;
  return (
    // 전역 ContentBackdrop(오행 배경색)이 비치게 투명(홈과 동일 처리).
    <View style={styles.bg}>
      {/* ── 검색 — 스크롤 **밖** 고정(daniel 08-06) ─────────────────────────
      ...
      <View style={[styles.searchBar, { paddingTop: insets.top + space(2) }]}>
      ...
      >
        {/* 검색 중에는 타이틀·명식·다음단계를 접는다 — 결과만 보이는 게 검색의 목적. */}
        {!searching && (
          <>
...
const styles = StyleSheet.create({
  ...
  wrap: { padding: space(5), paddingTop: space(3), paddingBottom: space(24) },
  ...
  // 검색 바 — 스크롤 **밖**(항상 보임). 스크롤과 겹치지 않고 위아래로 놓이므로 배경을 깔지 않는다
  //   (colors.bg 를 깔면 전역 ContentBackdrop 의 오행 배경색과 이 줄만 색이 어긋난다).
  searchBar: { paddingHorizontal: space(5), paddingBottom: space(2.5), backgroundColor: 'transparent' },
  searchBox: {
  ...
  // ★fontSize 와 lineHeight 는 반드시 짝(고정 lineHeight 면 글자확대 시 잘림 — daniel 07-28).
