  ...
  return (
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.wrap}>
      <Animated.View style={{ opacity: fadeAnim }}>
        ...
        {/* 통변 생성 진행률(daniel) — 풀이 중 홈으로 나와도 여기서 % 확인·탭하면 그 화면으로 이어보기 */}
        {gen.active && (gen.total > 0 && gen.done >= gen.total ? (
          // 완료(daniel 이슈13): 알림 끄지 말고 '풀이 보기' 버튼으로 — 탭하면 그 화면으로 이동 + 배너 닫기.
          <Pressable onPress={() => { const r = gen.route; setGenProgress({ active: false }); router.push(r as any); }} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: colors.ju, borderRadius: radius.md, paddingVertical: space(2.5), paddingHorizontal: space(4), marginBottom: space(3), gap: space(2) }}>
            <Text style={{ color: colors.bg, fontWeight: '800', fontSize: fs(13), flex: 1 }}>{gen.label} 풀이가 완성됐어요!</Text>
            <Text style={{ color: colors.bg, fontWeight: '800', fontSize: fs(13) }}>풀이 보기 ›</Text>
          </Pressable>
        ) : (
          <Pressable onPress={() => router.push(gen.route as any)} style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: colors.juSoft, borderColor: colors.ju, borderWidth: 1, borderRadius: radius.md, paddingVertical: space(2.5), paddingHorizontal: space(4), marginBottom: space(3), gap: space(2) }}>
            <Text style={{ color: colors.ju, fontWeight: '700', fontSize: fs(13), flex: 1 }}>{gen.label} 풀이 중… {gen.total > 1 ? `${gen.done}/${gen.total} (${Math.round((gen.done / gen.total) * 100)}%)` : '생성 중'}</Text>
