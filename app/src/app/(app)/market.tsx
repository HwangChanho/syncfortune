...
import { waitForPremium } from '../../lib/billing/premiumStore'; // 구매 후 서버(웹훅) is_premium 확인 대기(낙관적 표시 제거·daniel 07-05)
  ...
  async function buyPremium() {
    ...
    async function doBuyPremium(target: SavedChart | null) {
      ...
      try {
        ...
        // ★서버(rc-webhook)가 is_premium 세팅할 때까지 대기 후 반영(daniel 07-05) — 낙관적 표시(markPremiumOwnedNow) 제거로
        //   '결제 안 됐는데 적용된 것처럼' 현상 차단. 확인돼야만 프리미엄이 켜진다(서버=진실).
        ...
        if (confirmed) {
          ...
          // 웹훅 미도달(타임아웃) — 낙관적 표시 안 함. 결제는 완료됐으니 곧 자동 반영(포그라운드 재평가·다음 진입 시).
          await refresh();
