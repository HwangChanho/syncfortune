...
{/* 유료 상세(내 사주 깊이 보기) */}
        <DeepDiveCta kind="reading" label={t('joseonjob.detail', '내 사주 깊이 보기')} onPress={() => router.push({ pathname: '/reading', params: { input: JSON.stringify(me), kind: 'saju' } })} />
              {/* ★이어서 보면 좋은 콘텐츠(daniel 2026-07-27 "전부 붙여") — 화면마다 하단이 달라 보이던 것 통일.
            큐레이션 출처는 RELATED 단일(중복 하드코딩 0). 매핑이 없으면 스스로 아무것도 안 그린다. */}
        <RelatedContent kind="joseonjob" />
</ScrollView>
    </View>
  );
}
