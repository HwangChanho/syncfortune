...
export default function SelfAnalysisRoute() {
  ...
  const data = useMemo(() => {
    ...
    try {
      ...
      // 성격유형 120종(일간×월지) — 홈 주인공 히어로와 동일 산출(personaFromRepChart). 64종 6축 폐기·통합(daniel 2026-07-20).
      const persona = personaFromRepChart(input);
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.wrap}>
      ...
      {/* ② 성격유형 — 120종(일간×월지) 단일화(daniel 2026-07-20). 홈 히어로 상세(/personatype)와 같은 화면으로 잇는다. */}
      {persona && (
        <PressableScale style={styles.card} onPress={() => router.push('/personatype')}>
          ...
          <Text style={styles.personaDesc} numberOfLines={3}>{persona.summary}</Text>
          <Text style={styles.more}>{t('selfAnalysis.more', '자세히 보기 ›')}</Text>
        </PressableScale>
      )}
