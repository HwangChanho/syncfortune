...
// ── ⑦ 공용 지문이 개성을 **누르고 있지 않은가** (2026-08-26) ─────────────
//   ★실측이 근거다. Boss *"말투들이 너무 ai같고 딱딱해"* 의 원인을 재 보니:
//     · Boss 가 쓴 정답 예시 33건 — "당신" **0회** · 대개 1~2줄
//     · 실제 앱 답변 40건    — "당신" **79회** · 풍선 개수 3~4개로 **100% 고정**
//   말투에 호칭이 **이미 적혀 있었는데도** 어긴 것이다 ⇒ 공용 지문이 이겼다.
//   ⚠️★그리고 그 강제 문구는 **두 군데** 있었다(TALK_COMMON + 사용자 메시지 끝).
//     한 곳만 고치고 배포하면 아무것도 안 바뀐다 — 그래서 «파일 전체»에서 센다.
console.log('\n=== ⑦ 공용 지문이 개성을 누르지 않는가 (2026-08-26) ===');
{
  const edge = readFileSync('supabase/functions/talk/index.ts', 'utf8');
  // 주석은 뺀다 — 내 설명글이 검사에 걸리면 «고쳤다» 는 거짓 초록불이 된다(08-24 에 두 번 당함)
  const live = edge.split('\n').filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l)).join('\n');

  // ⑦-1 풍선 개수를 숫자로 강제하지 않는가 (사본 포함 — 파일 전체에서 센다)
  const forced = live.match(/말풍선\s*\d+\s*~\s*\d+\s*개|풍선\s*\d+\s*~\s*\d+\s*개/g) ?? [];
  if (forced.length) bad(`풍선 개수를 숫자로 강제한다(${forced.length}곳: ${forced.join(' · ')}) — 그러면 12명이 같은 리듬으로 말한다`);
  else ok('풍선 개수를 숫자로 강제하지 않는다 (길이는 할 말이 정한다)');

  // ⑦-2 "당신" 금지가 실제로 지시되는가
  if (/["“«]?당신["”»]?\s*(이라고|이라)?\s*부르지\s*마라|"당신" 이라 부르지 마라/.test(live)) ok('"당신" 호칭을 금지한다');
  else bad('"당신" 금지 문구가 없다 — 실측에서 답변 40건 중 31건이 "당신" 을 썼다');

  // ⑦-3 말투가 공용 규칙을 이긴다고 **말로** 못박았는가
  if (/어긋나는 것이 있으면 \*\*이 블록을 따른다/.test(live)) ok('말투가 공용 규칙보다 우선한다고 명시한다');
  else bad('말투 우선순위가 없다 — 3,500자 공용 지문이 300자 말투를 이긴다');

  // ⑦-4 안전 경계는 우선순위에서 **제외**되는가 (말투가 안전을 이기면 안 된다)
  if (/예외 없이 지킨다/.test(live)) ok('안전 경계는 말투 우선순위에서 제외된다');
  else bad('말투가 안전 경계까지 덮을 수 있다 — 의료·투자·법률 선이 무너진다');
}

// ── ⑧ 말투에 «입버릇·되묻기» 축이 있는가 ─────────────────────────────────
//   ★말끝만으로는 사람이 안 된다. 사람은 «아 그거…»·«음,»·«그랬군요.» 로 시작한다.
console.log('\n=== ⑧ 말투에 입버릇·되묻기 축이 있는가 ===');
{
  const miss = PERSONAS.filter((p) => !/입버릇/.test(p.persona) || !/되물을 땐/.test(p.persona));
  if (miss.length) bad(`축이 빠진 상담가 ${miss.length}명: ${miss.map((m) => m.name).join(' · ')}`);
  else ok(`${PERSONAS.length}명 전원에게 입버릇·되묻기가 있다`);

  // 되묻는 «내용»이 서로 다른가 — 전부 같으면 개성이 아니다
  const qs = PERSONAS.map((p) => (/되물을 땐(.*)/.exec(p.persona)?.[1] ?? '').slice(0, 40));
  const uniq = new Set(qs);
  if (uniq.size < PERSONAS.length) bad(`되묻는 방식이 겹친다(${PERSONAS.length}명 중 ${uniq.size}가지)`);
  else ok(`되묻는 방식이 ${uniq.size}가지로 전부 다르다`);
}

// ── ⑨ 음성 테스트 — ⑦⑧ 이 무뎌지면 잡히는가 ─────────────────────────────
console.log('\n=== ⑨ 음성 테스트 (⑦⑧ 기준이 살아 있는가) ===');
{
  const fake = 'const x = `★말풍선 3~4개(빈 줄 구분) · 한 풍선은 짧은 한 문장`;';
  const caught = (fake.match(/말풍선\s*\d+\s*~\s*\d+\s*개/g) ?? []).length > 0;
  caught ? ok('풍선 강제 문구를 되살리면 ⑦-1 이 잡는다') : bad('되살려도 못 잡는다 — ⑦-1 이 무디다');

  const cmt = '// ★말풍선 3~4개 를 걷어냈다';   // ⚠️주석은 잡으면 안 된다(거짓 빨간불)
  const live2 = cmt.split('\n').filter((l) => !/^\s*(\/\/|\*|\/\*)/.test(l)).join('\n');
  ((live2.match(/말풍선\s*\d+\s*~\s*\d+\s*개/g) ?? []).length === 0)
    ? ok('내 주석에 적힌 같은 문구는 잡지 않는다(거짓 빨간불 방지)')
    : bad('주석까지 물었다 — 08-24 와 같은 실수');

  const noPersona = PERSONAS.map((p) => ({ ...p, persona: p.persona.replace(/★입버릇.*/g, '') }));
  (noPersona.some((p) => !/입버릇/.test(p.persona)))
    ? ok('입버릇을 지우면 ⑧ 이 잡는다') : bad('지워도 통과한다 — ⑧ 이 무디다');
}
