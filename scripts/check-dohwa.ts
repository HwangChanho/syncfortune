...
//
// ★범위: 이 하네스는 **'값이 서로 일치하는가'만** 본다(A~D 삼합 도화표는 통변 서술·무료 티저·택일에서 여전히 쓴다).
//   ▷2026-07-20 daniel 확정으로 **재회·짝사랑 '시기' 관법이 바뀌었다**: 도화-충(구 B1)은 폐기되고,
//     시기는 **배우자성·배우자궁 × 세운(R-SPOUSE-DUAL)**이 담당한다("도화는 타고나는 것·발동 시점 없음").
//     → 유료 재회/짝사랑 timing 은 spouseDualCore(check:enginecopy 로 앱↔Edge 동기화)로 이관됨.
//   ▷그래도 삼합 도화표(A~D)는 남는다 — 통변 '서술'(R51 행위 결)·궁합 교차 도화(상대↔본인)·무료 티저·택일에서.
//     아래 A~D 일치 검사는 그 남은 쓰임들이 서로 어긋나지 않게 지키고, R51 카브아웃 검사는 "도화=시기 아닌 행위 /
//     시기=배우자성·궁×세운" 문구가 프롬프트에서 지워지지 않게 못 박는다(07-18 '삼합 축 잠정' → 07-20 폐기로 갱신).
//
...
ok(/시기.{0,2}가 아니라.{0,3}행위/.test(core) && /R-SPOUSE-DUAL|배우자성.{0,8}세운/.test(core),
  `★R51 층 분리(daniel 07-20 도화-충 폐기) — "도화=시기 아닌 행위 / 시기=배우자성·궁×세운(R-SPOUSE-DUAL)" 문구 보존`);
// 순일 판정 기준(전문가 2차 답변)이 빠지면 LLM 이 '무엇이 도화인가'를 스스로 지어내게 된다.
ok(/순일.{0,10}의 판정 기준/.test(core) && /극하는 글자를 만나면/.test(core),
  `★R51 순일 판정 기준 보존 — "한 기운 쏠림 + 극하는 세력 없음"`);
