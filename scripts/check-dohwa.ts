...
//   ※2026-07-18 전문가 2차 답변으로 근거가 더 명확해짐 — **"생지의 역마 발동은 시기가 아닌 행위에 초점"**
//     = 이 관법의 도화는 애초에 *시기 산출용이 아니다*. 그래서 R51(통변 서술)과 B1(삼합 도화 충 = 시기)은
//     경쟁 관법이 아니라 **층이 다르다**. 아래 카브아웃 체크는 그 층 분리가 지워지지 않게 지킨다.
//
...
ok(/시기.{0,2}가 아니라.{0,3}행위/.test(core) && /별개 관법/.test(core),
  `★R51 층 분리 유지 — "도화=시기 아닌 행위 / 시기 산출은 B1 별개 관법" 문구 보존`);
