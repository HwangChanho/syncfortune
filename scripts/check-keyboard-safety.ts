...
console.log('\n[포커스] 웹 대화창은 열자마자 입력칸에 커서가 가 있어야 한다');
{
  ...
  if (i < 0) fail('★대화창 자동 포커스가 없다 — 웹에서 열자마자 못 친다');
    ...
    // ⚠️★2026-08-27 **규칙이 바뀌었다**(Boss: *"상대가 대화중에 채팅이 막혀있는데
    //   중간에 계속 보낼수 있어야해"*). 종전 규칙은 «답하는 중엔 포커스를 건드리지 않는다» 였는데,
    //   그건 그때 `editable={!busy}` 로 **입력을 막고 있었기 때문**이다.
    //   이제 입력을 안 막으므로 포커스를 유지하는 편이 맞다 — 옛 규칙을 그대로 두면
    //   하네스가 **낡은 판단을 강제**한다([[harness-can-enforce-wrong-rule]]).
    //   ⇒ 지금 지켜야 할 것은 «답하는 중에 **입력을 막지 않는가**» 다.
    /editable=\{!busy\}/.test(body)
      ? fail('★답하는 중에 입력을 막는다 — Boss 지시(*"중간에 계속 보낼수 있어야해"*)와 어긋난다')
      : pass('답하는 중에도 **입력이 막히지 않는다**(모아서 한 턴으로 보낸다)');
  }
}
