...
//   ⚠️★★2026-08-27 — 이 줄 하나가 **하네스를 통째로 헛돌게** 하고 있었다.
//     `*` 로 시작하는 줄까지 버리는 바람에 여러 줄 주석의 **닫는 `*/` 줄이 사라졌다**
//       /**        ← 남고
//        * 설명     ← 버려지고
//        */        ← **버려진다**
//     그러면 여는 `/**` 만 남아 그 뒤가 **파일 끝까지** 블록 주석으로 먹힌다.
//     실측: 118,568자 → 89,855자, `type MyeongTab` 이 **통째로 사라져** 「하네스가 헛돈다」.
//     ⚠️위 주석은 *"다른 하네스들이 쓰는 순서와도 같다"* 고 적어 두었지만 **사실이 아니었다** —
//       나머지 다섯은 블록 주석을 **먼저** 지운다(그래서 `*` 로 시작하는 줄이 애초에 없다).
//       ★«같다» 고 적어 둔 것은 보장이 아니다([[duplicate-ui-single-source]]).
//   ⇒ 줄 주석은 `//` 로 시작할 때만 버린다. `*` 줄은 **블록 제거가 알아서** 걷는다.
