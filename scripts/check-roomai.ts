...
//   R2 사진: 2MB 상한이 있다
//   R3 사진: ⚠️**눌러서 되는 곳에만** 버튼을 그린다
//      ★2026-09-02 규칙을 다시 썼다 — 종전엔 «모바일에는 안 그린다» 였다.
//        그때는 `expo-image-picker` 가 없다고 **주석에 적혀 있었기** 때문인데, 실측하니
//        `package.json` 에도 `ios/Podfile.lock`(4곳) 에도 **이미 있었다.**
//        즉 낡은 사실을 하네스가 잠그고 있었고, Boss 요청(*"사진도 보낼수 있게 하라니깐"*)을 막았다.
//      ⇒ 취지는 그대로다: **죽은 버튼을 그리지 마라.** 다만 판정을 «플랫폼» 이 아니라
//        «**고를 수 있는가**»(`canPickImage`)로 바꾼다. 모듈이 없는 옛 빌드에서는 여전히 안 그린다.
...
// R3 — 눌러서 되는 곳에만 그린다(위 주석: 판정은 플랫폼이 아니라 «고를 수 있는가»)
{
  const guarded = /\(Platform\.OS === 'web' \|\| canPickImage\)\s*\?[\s\S]{0,260}?pickPhoto/.test(view);
  say(guarded, 'R3 ⚠️**눌러서 되는 곳에만** 사진 버튼을 그린다',
    guarded ? '' : '죽은 버튼을 두면 사용자는 «내 잘못» 으로 읽는다 — `canPickImage` 로 가려라');
}
