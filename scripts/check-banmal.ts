// scripts/check-banmal.ts — **반말/존댓말**이 나이로 정해지고, 말투를 잡아먹지 않는다
// ═══════════════════════════════════════════════════════════════════════════
...
//   B6. 인사도 반말 판이 있는가(인사만 존댓말이면 다음 말과 어긋난다)
//   B7. 안전 문장이 살아 있는가(무거운 자리에서 가볍게 굴지 마라 · 존댓말 요청 시 즉시 전환)
//
...
export function judgeEdge(code: string): Fail[] {
  ...
  // B7. 안전 문장
  if (!/존댓말로 해 달라고/.test(code)) out.push({ rule: 'B7', msg: '회원이 존댓말을 요청했을 때 되돌릴 길이 지시에 없다' });
...
// B3. 앱이 실제로 보내나
{
  ...
  if (talk == null || live == null) fail('B3', '앱 파일을 못 찾는다 — 경로가 바뀌었으면 이 하네스를 고칠 것');
    ...
    // ⚠️`open` 이 `myAge` 를 의존성에 안 넣으면 **인사만 존댓말로 굳는다**
    if (!/\}, \[t, dateKey, myName, bumpChats, myAge\]\)/.test(tc)) {
      fail('B6', `${TALK} — open 의 의존성에 myAge 가 없다. 나이는 나중에 들어오므로 **인사가 영원히 존댓말**이 된다`);
    }
  }
}
// B6. 인사 반말 판
{
  ...
  if (g == null) fail('B6', `${GREET} 이 없다`);
    ...
    if (!/LINE_CASUAL/.test(code)) fail('B6', `${GREET} — 반말 인사가 없다. 인사만 존댓말이면 다음 말과 어긋난다`);
      ...
      const seg = code.slice(code.indexOf('LINE_CASUAL'), code.indexOf('export function greetingFor'));
      const politeLeft = [...seg.matchAll(/'([^']*(?:하세요|해요|이에요|세요\?|예요)[^']*)'/g)].map((m) => m[1]);
